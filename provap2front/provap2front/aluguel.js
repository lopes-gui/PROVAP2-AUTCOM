...
function aluguel(clientId, carId) {
    ...
    .then(([clientResponse, carResponse]) => {
        if (clientResponse.ok && carResponse.ok) {
            // Cliente e carro existem, pode prosseguir com o aluguel

            // Aqui você pode realizar a lógica para registrar o aluguel na sua API
            // Por exemplo:
            const aluguelData = {
                clientId: clientId,
                carId: carId,
                data: new Date().toISOString() // Data atual
                // Outros dados do aluguel, se necessário
            };
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    // Você pode querer atualizar a visualização dos aluguéis após realizar o aluguel
                    // viewAlugueis();
                } else {
...
function deleteAluguel(id) {
    ...
    .then((response) => {
        if (response.ok) {
            ...
            // Atualizar a visualização dos aluguéis após a exclusão
            // viewAlugueis();
        } else {
        ...
        if (clientResponse.ok && carResponse.ok) {
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    // Atualizar a visualização dos aluguéis após a atualização
                    // viewAlugueis();
                } else {
